...
public class Account {
	...
	public void setUpdateDataNow() {
		if(Objects.nonNull(this.updateData)) return;
	...
	public String checkAccountData() {
		...
		if(StringUtils.isEmpty(this.accountName)) accountErrorMessage.add("アカウント名");
		
		if(StringUtils.isEmpty(this.postCode)) accountErrorMessage.add("郵便番号");
		
		if(StringUtils.isEmpty(this.address)) accountErrorMessage.add("住所");
		
		if(StringUtils.isEmpty(this.telephoneNumber)) accountErrorMessage.add("電話番号");
		
		if(!CollectionUtils.isEmpty(accountErrorMessage)) return String.join(",", accountErrorMessage) + "が未入力です";
